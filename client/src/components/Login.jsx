...
export default function Login({setUser}) {
  ...
  const login = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_API_ROUTE}/users/login`, { email, password });
      setUser(res.data);
    ...
    <div className="overlay">
      <div className="auth flex-col gap-16 align-center absolute-center">
        ...
        <form onSubmit={login} className="flex-col gap-16">
          ...
          <div className="input flex-col gap-4">
            <label htmlFor="password" className="required flex gap-4">Password <div className="hidden error"> - does not match</div></label>
            <input
              type="password"
              name="password"
              id="password"
        ...
        <div className="auth-redirect">Already have an account? <Link to="/signup" className="auth-redirect">Register</Link></div>
      </div>
    </div>
  );
}
